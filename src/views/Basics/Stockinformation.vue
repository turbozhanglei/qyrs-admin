      ...
      <el-form :inline="true" :model="filters" :size="size" ref="filters" class="form">
        <el-row>
          ...
          <el-form-item prop="is_expired_days">
            <el-select
              v-model="filters.is_expired_days"
              style="width: 100%;"
              placeholder="是否过期"
              clearable
            >
          ...
          <el-form-item prop="bill">
            <el-input v-model="filters.bill" placeholder="单据"></el-input>
          </el-form-item>
  ...
  methods: {
    ...
    downloadExcel() {
      this.$confirm("确定下载列表文件?", "提示", {
        ...
        .then(() => {
          this.exportData(this.filters, this.filterColumns, "库存管理");
        })
        .catch(() => {});
    },
    //excle导出
    ...
    findPage: function(data) {
      ...
      if (
        this.filters.is_expired_days != "" &&
        this.filters.is_expired_days == 0
      ) {
        //未过期
        this.filters.min_is_expired_days = 0;
        this.filters.max_is_expired_days = "";
      } else if (this.filters.is_expired_days == 1) {
        //过期
        this.filters.min_is_expired_days = undefined;
        this.filters.max_is_expired_days = -1;
      } else {
        this.filters.min_is_expired_days = "";
        this.filters.max_is_expired_days = "";
      }
